    ...
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/60 to-transparent p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-white text-sm font-mono tabular-nums bg-black/40 px-2 py-0.5 rounded">
            {formatTime(elapsed)}
          </span>
        </div>
        {exercise && (
      ...
      <div className="absolute top-16 right-3 flex flex-col items-end gap-2">
        {formScore !== null && (
          <div className={`${scoreColor} rounded-lg px-3 py-2 text-center min-w-[60px]`}>
            <span className="text-white text-xl font-bold block tabular-nums">{formScore}</span>
            <span className="text-white/80 text-[10px]">FORM</span>
          </div>
        )}
        <div className="bg-black/50 rounded-lg px-3 py-2 text-center min-w-[60px]">
          <span className="text-white text-xl font-bold block tabular-nums">{reps}</span>
